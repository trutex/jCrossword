(function ($) {
    ...
    $.crossword = function (data, options) {
        return $.crossword.impl.init(data, options);
    }

    $.fn.crossword = function (options) {
        return $.crossword.impl.init(this, options);
    }
    ...
    $.crossword.impl = {
        ...
        buildGrid: function (options) {
            ...
            grid.find('td.tile-active').each(function () {
                ...
                if (acrossNo) {
                    ...
                    acrossClues.push(s.buildClueObject(options.acrossClues[acrossClueIndex], clueNo, (col - j)));
                    acrossClueIndex++;
                }
                ...
                if (downNo) {
                    ...
                    // Convert clue array into an object array.
                    downClues.push(s.buildClueObject(options.downClues[downClueIndex], clueNo, (row - i)));
                    //options.downClues[downClueIndex] = s.buildClueObject(options.downClues[downClueIndex], clueNo, (row - i));
                    downClueIndex++;
                }
        ...
        buildClueObject: function (clue, clueNo, clueLength) {
            ...
            if(clueParts.length > 1){
                ...
                multiClues.push( { clueNo: clueNo, multi: clueParts.slice(1).join() } );
            }
            ...
            return { text: clueText, number: clueNo, length: clueLength, answer: clueAnswer, pattern: cluePattern };
        },
                    ...
                    if (code === 8) {
                        ...
                        if (/[A-Z]/i.test(charEntered)) {
                            ...
                            if (cursorIndex < highlightedTiles.length) {
                                ...
                                var findClueAnswer = function (clueArray, clueNo) {
                                    var clue = $.grep(clueArray, function (c) { return c.number === clueNo });
                                    return clue.length === 1 && clue[0].answer ? clue[0].answer.toUpperCase() : '';
                                };
                                // Across clue is highlighted, look for answer in across clues array.
                                if (direction === 'a') {
                                    var acrossClueNo = parseInt(highlightedTiles.last().attr('acrossClueNo'));
                                    actualAnswer = findClueAnswer(acrossClues, acrossClueNo);
                                }
                                else {
                                    // Down clue is highlighted, look for answer in down clues array.
                                    var downClueNo = parseInt(highlightedTiles.last().attr('downClueNo'));
                                    actualAnswer = findClueAnswer(downClues, downClueNo);
                                }
        ...
        highlightTiles: function (tile) {
            var prevHighlight = 0;
            // Record which direction the clue is currently hightlighted in, if any - 1=across, 2=down
            ...
            if (acrossClueNo && prevHighlight === 0) {
                $(this.stringFormat('#grid td[acrossClueNo={0}]', acrossClueNo)).addClass('tile-highlight');
                highlightedClue = acrossClueNo + 'a';
            }
            else
            // Highlight the down clue.
                if (downClueNo && prevHighlight !== 2) {
                    $(this.stringFormat('#grid td[downClueNo={0}]', downClueNo)).addClass('tile-highlight');
